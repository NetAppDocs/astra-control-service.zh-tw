...
適用於Google Cloud的支援提供兩種服務類型：_CVS_和_CVs-Performance。Cloud Volumes Service這些服務類型在特定的Google Cloud地區均受到支援。 https://["前往NetApp Cloud Central的全球區域地圖"^] 識別叢集所在的Google Cloud區域所支援的服務類型。
...
| CVS效能 | 具有中高儲存效能需求的應用程式 | https://["檢視支援的Google Cloud地區"^]  a| 
...
| CVS | 需要低到中儲存效能的應用程式 | https://["檢視支援的Google Cloud地區"^] | NetApp-CVS標準 | 300 GiB 
|===
...
https://["如需深入瞭解這些服務層級、請參閱Cloud Volumes Service 《for Google Cloud》（英文）文件"^]。



==== 持續的Volume大小與效能

https://["如Google Cloud文件所說明"^]、每個服務層級的允許頻寬是以已配置容量的GiB為單位。這表示較大的磁碟區將提供較佳的效能。
...
https://["深入瞭解Cloud Volumes Service 《適用於Google Cloud的解決方案》文件中的標準服務層級"^]。
...
https://["深入瞭解Google持續磁碟"^]。

https://["深入瞭解不同的Google持續磁碟效能等級"^]。
