...
[role="lead"]
您可以使用 Astra Control Service 來管理 Astra 中的私有 Azure Kubernetes 服務（ AKS ）和 Red Hat OpenShift 叢集。這些指示假設您已建立私有 aks 或 OpenShift 叢集、並準備了安全的方法來遠端存取；如需建立及存取私有 aks 或 OpenShift 叢集的詳細資訊、請參閱下列文件：
