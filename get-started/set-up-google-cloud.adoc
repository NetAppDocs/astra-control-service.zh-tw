...
私有叢集:: 如果叢集為私有、則為 https://["授權網路"^] 必須允許Astra控制服務IP位址：
+
--
52.188.218.166/32
...
.步驟
. 前往 https://["NetApp Cloud Volumes Service 技術頁面"^] 在Google Cloud Marketplace中、選取* Purchase *、然後依照提示進行。
+
https://["請依照Google Cloud文件中的逐步指示來購買及啟用服務"^]。
...
.步驟
. https://["使用Google Cloud主控台或gclCloud CLI啟用下列API"^]：
+
** Google Kubernetes Engine
** 雲端儲存設備
** Cloud Storage Json API
...
.步驟
. 前往Google Cloud和 https://["使用主控台、gCloud命令或其他慣用方法來建立服務帳戶"^]。
. 授予服務帳戶下列角色：
+
...
.步驟
. 前往Google Cloud和 https://["使用主控台、gCloud命令或其他慣用方法來建立服務帳戶金鑰"^]。
...
.步驟
. https://["前往NetApp Cloud Central的全球區域地圖"^] 並識別您將在叢集所在的Google Cloud區域中使用的服務類型。
+
提供兩種服務類型：CVS和CVs-Performance。Cloud Volumes Service https://["深入瞭解這些服務類型"^]。

. https://["前往Google Cloud Platform的Cloud Volumes"^]。
...
+
如需使用這些命令的詳細資訊、請參閱 https://["適用於GCP的QuickStart Cloud Volumes Service"^]。

+
https://["深入瞭解如何設定私有服務存取及設定網路對等"^]。
